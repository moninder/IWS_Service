...
namespace AirportIQ.IWS.Service.API
{
	...
	using AirportIQ.IWS.Service.API.IwsService; // real
	 ...
	 * ****************/
	[TestFixture]
	public class IwsServiceAPITest
	{

		static IwsServiceAPI testSvc;
		...
		static byte[] testImage = { 1, 2, 3, 4, 5 };
		...
		public IwsServiceAPITest()
		{
			testSvc = new IwsServiceAPI();
		}
      ...
      //   Assert.IsTrue(testSvc.(IWS_CardID));
      //}
      ...
      [Test, Description("T02")]
		...
		public void InitiateBackgroundCheck()
		{
			...
			Assert.IsTrue(testSvc.InitiateBackgroundCheck(BC_PersonID, TSCTransactionTypeID, TransactionControlNumber, TransactionDate, ProgramIdentification, 
											ResponseIdentification, Status, StatusText, XMLdata, Direction, true));
		}
		...
		public void UpdateBackgroundCheck()
		{
			...
			Assert.IsTrue(testSvc.UpdateBackgroundCheck(BC_PersonID, AgencyCode, CheckTypeCode, TransactionTypeCode, TransactionControlNumber,
											TransactionDate, Result, ResultDate, ResultDetails, ResultDetailDate, true));
		}
      ...
      [Test]
      public void SetTSACaseNumber()
      {
         Guid iwsPersonID = new Guid("89C29FFE-FFC3-F10A-42B6-D3BEAD98A923");
         string caseNumber = "this is the case number";
         Assert.IsFalse(testSvc.SetTSACaseNumber(iwsPersonID, caseNumber, "PASS", new DateTime()));
      }
